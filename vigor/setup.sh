...
if [ ! -e "$BUILDDIR/klee" ]; then
  git clone --depth 1 https://github.com/sosp21paper780/vigor-klee.git "$BUILDDIR/klee"
  pushd "$BUILDDIR/klee"
    rm -rf build
    mkdir build
    pushd build
...
if [ ! -e "$BUILDDIR/libr3s" ]; then
  git clone --depth 1 https://github.com/sosp21paper780/libr3s.git "$BUILDDIR/libr3s"
